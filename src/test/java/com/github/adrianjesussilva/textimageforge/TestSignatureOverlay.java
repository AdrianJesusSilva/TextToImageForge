		...
		try {
			...
			for (String linea : lineas) {
				...
				if (linea.startsWith(" ", 1)) {
					...
					forge.addLine(TextForge.builder().text(linea + "    ").font(FUENTE).color(Color.BLACK).textAlign(TextAlign.CENTER).build());
				} else if (linea.equals("")) {
					forge.addLine(TextForge.builder().text(" ").textAlign(TextAlign.CENTER).font(FUENTE).color(Color.BLACK).build());
				} else {
					forge.addLine(linea + "    ");
				}
			}
			...
			forge.forgeImage(archivo, ImageEncoder.png, (firma!= null && !firma.isEmpty()? firma: null));
